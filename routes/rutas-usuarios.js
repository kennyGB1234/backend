...
// * Listar todos los coches alquilados de un id de usuario
// router.get("/cochesAlquilados", async (req, res, next) => {

//   if (!userId) {
//     const error = new Error("Se requiere autenticación para acceder a esta ruta");
//     error.code = 401;
//     return next(error);
//   }
...
//   let usuario;
//   try {
//     const decodeduserId = jwt.verify(userId, process.env.JWT_SECRET);
//     usuario = await Usuario.findById(decodeduserId.userId).populate("coches");
//   } catch (err) {
//     const error = new Error("Ha ocurrido un error en la recuperación de datos");
//     error.code = 500;
//     return next(error);
//   }
//   res.status(200).json({
//     mensaje: `Coches del usuario con ID ${decodeduserId.userId}`,
//     coches: usuario.coches,
//   });
// });

router.get("/cochesAlquilados/:id", async (req, res, next) => {
 let usuario=req.params.id;

  let coche;
  try {
    coche = await Usuario.findById(usuario).populate("coches");
  } catch (err) {
